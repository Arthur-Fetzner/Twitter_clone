...
class Usuario extends Model{
    ...
    public function autenticar(){
        ...
        if(count($usuario) == 1){
            $this->__set('id', $usuario[0]['id']);
            $this->__set('nome', $usuario[0]['nome']);
        }
    ...
    public function getMaisSeguidos(){
        $query = "select 
            id_usuario_seguindo as id,
            count(id_usuario_seguindo) as seguidores,
            (
                select nome
                from usuarios
                where id = id_usuario_seguindo
            ) as nome
        FROM 
            usuarios_seguidores
        GROUP BY 
            id_usuario_seguindo
        ORDER BY 
            count(id_usuario_seguindo) DESC 
        LIMIT 10";
        $stmt = $this->db->prepare($query);
        $stmt->execute();
